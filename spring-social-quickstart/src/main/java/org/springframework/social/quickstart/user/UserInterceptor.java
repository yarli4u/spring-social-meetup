...
public final class UserInterceptor extends HandlerInterceptorAdapter {
	...
	private void handleSignOut(HttpServletRequest request, HttpServletResponse response) {
		if (SecurityContext.userSignedIn() && request.getServletPath().startsWith("/signout")) {
			connectionRepository.createConnectionRepository(SecurityContext.getCurrentUser().getId()).removeConnections("meetup");
			userCookieGenerator.removeCookie(response);
			SecurityContext.remove();			
		}
	}
	...
	private boolean userNotFound(String userId) {
		...
		return connectionRepository.createConnectionRepository(userId).findPrimaryConnection(Meetup.class) != null;
	}
